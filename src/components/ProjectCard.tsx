  ...
  return (
    ...
    >
      ...
      <div className="w-full lg:w-2/3 p-4 md:p-8 relative flex items-center justify-center bg-[var(--color-background)]">
        <div className="w-full aspect-[4/3] md:aspect-[16/9] bg-white/5 overflow-hidden border border-[var(--color-border)] flex items-center justify-center relative">
          ...
          <div className="absolute top-4 right-4 z-10 flex gap-2 pointer-events-none">
             {project.technologies.slice(0, 3).map((tech) => (
               <span key={tech} className="px-2 py-1 bg-[#0A0A0A]/80 border border-[var(--color-border)] backdrop-blur-md text-white text-[9px] font-mono uppercase tracking-widest rounded-none">
                 {tech}
               </span>
             ))}
          </div>
